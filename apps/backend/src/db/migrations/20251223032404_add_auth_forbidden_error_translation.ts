...
/**
 * Migration to add/update AUTH_FORBIDDEN error translation
 * Specifically adds: errors.AUTH_FORBIDDEN
 */
export async function up(knex: Knex): Promise<void> {
  ...
  console.warn("Starting import of AUTH_FORBIDDEN error translation...");
  ...
  // Only load translation for the key we care about
  const targetKey = "errors.AUTH_FORBIDDEN";

  for (const language of SUPPORTED_LANGUAGES) {
    const flatTranslations = loadTranslationFile(language, NAMESPACE);

    const value = flatTranslations[targetKey];
    if (value) {
      translations.push({
        namespace: NAMESPACE,
        key_path: targetKey,
      ...
      console.warn(`Missing translation for ${language}: ${targetKey}`);
    }
  }
...
export function down(_knex: Knex): Promise<void> {
  // Optionally remove the translation key
  // This would delete the specific key we added
  console.warn(
    "Down migration: AUTH_FORBIDDEN translation key remains in database (manual cleanup if needed)",
  );
