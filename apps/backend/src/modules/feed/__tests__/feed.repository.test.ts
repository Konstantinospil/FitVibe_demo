...
describe("Feed Repository", () => {
  ...
  describe("findActiveShareLinkBySession", () => {
    it("should find active share link for session", async () => {
      const mockLink = {
        id: "link-123",
        session_id: "session-123",
        token: "abc123",
      ...
      mockQueryBuilder.first.mockResolvedValue(mockLink);

      const result = await feedRepository.findActiveShareLinkBySession("session-123");
  ...
  describe("findShareLinkByToken", () => {
    it("should find share link by token", async () => {
      const mockLink = {
        id: "link-123",
        token: "abc123",
        session_id: "session-123",
      ...
      const result = await feedRepository.findShareLinkByToken("abc123");

      expect(result).toEqual(mockLink);
      expect(mockQueryBuilder.where).toHaveBeenCalledWith({ token: "abc123" });
    });
  });

  describe("insertShareLink", () => {
    it("should insert share link", async () => {
      const mockLink = {
        id: "link-new",
        session_id: "session-123",
        feed_item_id: null,
      ...
      mockQueryBuilder.returning.mockResolvedValue([mockLink]);

      const result = await feedRepository.insertShareLink({
        sessionId: "session-123",
        feedItemId: null,
        token: "newtoken",
        createdBy: "user-123",
  ...
  describe("incrementShareLinkView", () => {
    it("should increment view count", async () => {
      await feedRepository.incrementShareLinkView("link-123");

      expect(mockQueryBuilder.where).toHaveBeenCalledWith({ id: "link-123" });
      expect(mockQueryBuilder.increment).toHaveBeenCalledWith("view_count", 1);
    });
  });

  describe("revokeShareLinksForSession", () => {
    it("should revoke all active links for session", async () => {
      mockQueryBuilder.update.mockResolvedValue(2);

      const result = await feedRepository.revokeShareLinksForSession("session-123");
