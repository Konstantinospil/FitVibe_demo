...
| Accessibility                  | WCAG 2.2 AA, axe/Lighthouse score **≥90** (Auth, Planner, Logger, Dashboard)                                      | a11y JSON & screenshot notes                |
...
We maintain a living RTM at `docs/1.Product_Requirements/rtm_comprehensive.csv`, mapping **PRD FR/NFR → AC IDs → Test IDs/paths → Evidence artifacts**.
The **AC master** lives in `docs/1.Product_Requirements/e.Acceptance_Criteria/INDEX.md` (reviewed with PRD updates). CI checks ensure referenced tests exist and are green.
