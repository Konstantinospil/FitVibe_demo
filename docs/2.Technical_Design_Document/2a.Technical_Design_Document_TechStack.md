...
| --- | --- | --- | --- |
| FR-001 / FR-002 / FR-003 | Registration, login/session, auth wall | §3, §4.1, §5.1, §7 | Registration/login/session/auth-wall verification |
| FR-004 | Planner | §4.4, §6, §7 | Planning/session workflow |
| FR-005 | Logging & Import | §4.4, §6, §7 | Logging/import verification |
| FR-006 | Gamification | §4.6, §6, §7 | Points/badges rules |
| FR-007 | Analytics & Export | §4.5, §6, §7 | Aggregation/export verification |
| FR-008 | Admin & RBAC | §4, §5.1, §7 | Role/authorization/admin audit verification |
| FR-009 | Profile & Settings | §4.2, §6, §7 | Profile/preferences |
| FR-010 | Exercise Library | §4.3, §6, §7 | Exercise ownership/search/history |
| FR-011 | Sharing & Community | §4.7, §5.1/§5.2, §7 | Visibility/feed/social controls |
| FR-012 | Coach Training Unit Assignment | domain modules/schema/API | Coach/athlete assignment workflow |
| FR-014 | Profile Measurements | profile/data/API design | Measurement definitions/values/discovery/derived metrics |
...
| Accessibility | WCAG 2.2 AA                                 | §5.5                               | Automated checks (CI)                   |
...
- **In:**
  ...
  - RBAC roles (`athlete`, `coach`, `support`, `admin`);
  - email delivery;
...
The [Context Diagramm](./2.g.Diagrams/tdd-2-1-context-diagram.mmd) can be found in the diagrams.
...
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Frontend (Runtime & UI)**       | - **React 18 + Vite** (SPA shell, fast dev/prod builds)<br>- **React Router DOM** (client routing)<br>- **Axios** with interceptor (`401 → /auth/refresh`)<br>- **State Management**: Zustand (lightweight) or Redux (global orchestration); optional React Query for caching & background refetch<br>- **TailwindCSS** (design system, theming tokens)<br>- **Recharts** (charts for dashboards)<br>- **Lucide React** (icons)<br>- **Accessibility**: WCAG 2.2 AA, ARIA, keyboard navigation<br>- **Internationalization**: EN/DE/EL/ES/FR tokens |
| **Backend (Runtime & Framework)** | - **Node.js 24 LTS + Express** with modular routers (`auth`, `users`, `exercises`, `sessions`, `progress`, `points`, `feed`)<br>- **Knex.js** for SQL queries, migrations, seeds<br>- **Argon2id** for target-state password hashing; temporary bcrypt verification only for migration compatibility<br>- **jsonwebtoken (RS256)** for short-lived access tokens<br>- **Refresh tokens**: server-stored, rotated by `jti`, revocable on logout<br>- Optional **TOTP 2FA** with backup codes                                                                                                                                      |
| **Authentication & Security**     | - **Helmet** for HTTP headers<br>- **CORS allowlist** (env-configured)<br>- **express-rate-limit** (per-IP & per-account throttling with opaque pre-auth responses; no CAPTCHA/puzzle requirement)<br>- **CSRF protection** middleware<br>- **httpOnly Secure cookies** for refresh/session<br>- **compression** for gzip responses                                                                                                                                                                                                                                                             |
...
- **Node.js 24 LTS + Express**: Chosen for an actively supported LTS (Node 20 reached EOL in April 2026), wide ecosystem support, and straightforward modularization. Express offers fine-grained control, which is preferable over heavier frameworks for a project emphasizing separation of concerns.
...
- **Argon2id**: Target password-hashing algorithm per the accepted Password & Authentication Policy. Existing bcrypt hashes may be verified during the migration window and upgraded after successful authentication; new/reset/changed passwords use Argon2id in the target design.
...
- **Internationalization (placeholder):** support EN/DE/EL/ES/FR labels with English fallback.
...
- **Backend:** Node.js 24 LTS, TypeScript, Express, zod (validation), jose (JWT), nodemailer (email), rate-limiter-flexible.
...
- **IDs:** UUID v4 for time-sortable identifiers; no sequential IDs exposed.
- **Locale/i18n:** The supported UI locale set is **EN/DE/EL/ES/FR**; `Accept-Language` and user settings negotiate locale; fallback to EN; dates/numbers formatted per locale on the client (server provides ISO values + hints where applicable).
...
- **i18n scope (MVP):** Static/system catalogs cover **EN/DE/EL/ES/FR**; English is the fallback.
