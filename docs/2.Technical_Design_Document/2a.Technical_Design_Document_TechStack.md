...
| **Database & Persistence**        | - **PostgreSQL 18** (primary relational datastore)<br>- **Knex.js** migrations (`YYYYMMDDHHMM__*.ts`)<br>- Constraints & indexes (e.g., `lower(username)`, partial indexes, FK cascades)<br>- Soft deletes (`deleted_at`, `archived_at`)<br>- **Encryption in Transit**: TLS 1.3/1.2 with certificate verification in production (`PGSSL=true`, `PGSSLMODE=require` or `verify-full`)<br>- **Encryption at Rest**: Encrypted storage volumes (encrypted EBS, encrypted Kubernetes PersistentVolumes, or cloud provider managed encryption)<br>- **Backups**: daily encrypted (AES-256), RPO ≤ 24h, RTO ≤ 4h<br>- **Retention policies** per entity, optional anonymization for analytics |
...
- **Transport Security**:
  ...
  - **Database SSL/TLS**: All database connections use TLS 1.3 (minimum TLS 1.2) with certificate verification in production. Environment-specific configuration allows relaxed SSL for development.
- **Database Encryption**:
  - **Encryption in Transit**: TLS/SSL encryption for all database connections with certificate verification in production (see [ADR-023](../2.f.Architectural_Decision_Documentation/ADR-023-database-encryption.md)).
  - **Encryption at Rest**: Filesystem-level encryption for database volumes (encrypted EBS volumes, encrypted Kubernetes PersistentVolumes, or cloud provider managed encryption).
  - **Backup Encryption**: All database backups encrypted using AES-256.
  - **Performance**: Encryption overhead target ≤5% (measured via benchmarks).
  - **Key Management**: Encryption keys managed according to KEY_MANAGEMENT_POLICY.md (rotation every 6 months).
