...
> Base URL: `https://api.fitvibe.app/api/v1` (staging/prod) - `http://localhost:4000/api/v1` (local)
...
All fields are optional. Supports updating profile fields including alias, weight, fitness level, and training frequency (FR-009).
...
- `alias`: 3-50 characters, URL-safe (letters, numbers, underscores, dots, dashes), case-insensitive unique
- `weight`: Positive number, 20-500 kg (converted from lb if `weightUnit` is "lb")
- `weightUnit`: Enum `"kg"` or `"lb"` (default: `"kg"`)
- `fitnessLevel`: Enum `"beginner"`, `"intermediate"`, `"advanced"`, `"elite"`
- `trainingFrequency`: Enum `"rarely"`, `"1_2_per_week"`, `"3_4_per_week"`, `"5_plus_per_week"`

**Response 200 OK:**

Includes updated profile with latest metrics:
...
- `400 Bad Request`: Validation error (invalid format, out of range)
- `401 Unauthorized`: Missing or invalid authentication
- `409 Conflict`: Alias already taken (case-insensitive)
...
| **Accessibility**      | WCAG 2.2 AA                                                                           | §8.5 a11y rules                                                                         | axe CI checks, manual keyboard walk-through for key flows                            |
