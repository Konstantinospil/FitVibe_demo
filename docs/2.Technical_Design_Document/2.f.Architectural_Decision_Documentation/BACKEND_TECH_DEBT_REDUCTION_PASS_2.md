...
- exit-gate CI exposed two repair-induced contract failures and one underlying schema drift. Frontend badge API tests still asserted the obsolete routes and were updated to the repaired contract. More importantly, the runtime pending-2FA repository uses `failed_attempts`/`last_failed_at` but the base migration did not create those columns; the migration and migration-contract test now align with the runtime model.
