...
- Monorepo with **pnpm** workspaces; Node.js 24 LTS; React 18; PostgreSQL â‰¥ 14 (target 16-18 locally).
...
1. **`quality`** job:
   - Prepare: checkout, pnpm cache restore, Node.js 24 setup, native dependencies
...
- **Goal:** Achieve QA gates (coverage â‰¥ **80%**, perf **p95 < 300 ms**, a11y **WCAG 2.2 AA**) with fast, reliable tests.
...
| `BASE_URL`                        | url                                               |                Yes | `http://localhost:4000` | external base URL                          |
...
- **Location**: `docs/2.Technical_Design_Document/2.f.Architectural_Decision_Documentation/ADR-XXX-title.md` (three-digit, zero-padded index).
...
All ADR IDs must **match the filename** under `docs/2.Technical_Design_Document/2.f.Architectural_Decision_Documentation/` and every accepted ADR must link back to the exact sections it changes (PRD/TDD/QA).
...
| ADR-020 | Accessibility Compliance (WCAG 2.2 AA) & Inclusive UX                                    | ✅ Accepted | Semantics-first components, keyboard flows, contrast, alt text; Lighthouse/axe in CI with ≥90 score.                                                 | 2025-10-14    | [ADR-020](./ADR-020-accessibility-compliance.md)              |
| ADR-021 | Standardize Backend Test Runner on Jest 30 + @swc/jest                                   | ✅ Accepted | Adopt _Jest 30_ with _@swc/jest_ for the backend test runner (unit + integration with Supertest), replacing ts-jest.                                 | 2025-10-14    | [ADR-021](./ADR-021-test-runner-backend-jest30-swc.md)        |

> When an ADR is **Accepted**, update the `Status` column and add its file path in the Links cell (e.g., `docs/2.Technical_Design_Document/2.f.Architectural_Decision_Documentation/ADR-001-api-versioning.md`).
...
1. **Create file** `docs/2.Technical_Design_Document/2.f.Architectural_Decision_Documentation/ADR-XYZ-title.md` from the template in §15.1.1.
