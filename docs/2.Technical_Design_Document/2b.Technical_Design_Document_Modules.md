...
- **Identifiers:** UUID (RFC 4122 v4 via crypto.randomUUID), immutable.
...
See also [State flows](./2.g.Diagrams/tdd-4-1-2-states-flows.mmd).
...
- **Update profile:** partial DTO; server validates lengths/formats; alias change rate-limited (e.g., 1/30d). Supports updating alias, weight (with unit conversion), fitness level, and training frequency (FR-009).
- **Get profile:** own profile (full) including latest metrics (weight, fitness level, training frequency), others' profile (respecting visibility).
...
- Unique `(LOWER(alias))`; min 3, max 50 chars; URL-safe (letters, numbers, underscores, dots, dashes); reserved names blocked.
...
- **Validation & Storage:**
  ...
  - Acceptable `weight` range (kg-equivalent): **20–500**.
