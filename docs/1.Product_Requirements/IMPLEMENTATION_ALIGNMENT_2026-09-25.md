...
| E19 / US-19.1 Publication Snapshots | Open | Backoffice translation authoring exists. | ADR-032 immutable publication snapshots, minor/authoritative classification and governed publication are not implemented. |
| E19 / US-19.2 Registration Acceptance | Progressing | Registration checkbox and acceptance persistence exist. | Acceptance currently stores calculated mutable version, not authoritative snapshot identity required by ADR-032. |
| E19 / US-19.3 Re-Acceptance | Progressing | Re-acceptance UI/status/service exist. | Trigger currently derives from calculated live version; must move to authoritative snapshot semantics. |
