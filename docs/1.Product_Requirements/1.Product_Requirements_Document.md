...
Accessibility shall be central, following **WCAG 2.2 AA standards**, providing a barrier-free experience across devices and user groups.
...
- Ensure an **accessible and responsive UI** (WCAG 2.2 AA).
...
- Customizing
  - **Multilanguage**: English (`en`), German (`de`), Greek (`el`), Spanish (`es`), and French (`fr`) for supported UI/system text, with English fallback.
...
- [Athlete](../3.Sensory_Design_System/3.a.Personas/persona-member-athlete.md)
- [Follower](../3.Sensory_Design_System/3.a.Personas/persona-follower.md)
- [Coach](../3.Sensory_Design_System/3.a.Personas/persona-coach-trainer.md)
- [Administrator](../3.Sensory_Design_System/3.a.Personas/persona-administrator.md)
- [Content Curatoer](../3.Sensory_Design_System/3.a.Personas/persona-content-curator.md)
...
- **Registration** with verified email (15-minute TTL token, 3 resend/hour per user, auto-purge in 7 days).
...
- **Password policy:** ≥12 characters; passphrases are supported; no mandatory character-class composition rules; common/breached passwords are rejected according to the Password & Authentication Policy.
- **Brute force protection:** temporary throttling/backoff and server-side abuse controls with opaque pre-authentication responses; no CAPTCHA/puzzle-style cognitive authentication requirement.
...
- **Roles & authorization:** canonical roles are `athlete`, `coach`, `support`, and `admin`, enforced server-side.
...
- New/changed passwords target **Argon2id** per the Password & Authentication Policy. During migration, existing bcrypt hashes may be verified only for compatibility and are re-hashed to Argon2id when safely possible.
...
- No CAPTCHA/puzzle-style cognitive challenge is required; abuse resistance uses throttling, opaque responses, second-factor controls, and recovery mechanisms.
...
| --- | --------------------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| R1  | Brute-force login attempts        | Automated password guessing against login endpoint.                               | - **Rate limiting/throttling** per IP & account.<br>- Opaque pre-authentication responses.<br>- MFA/recovery controls without cognitive puzzles.<br>- Strong password policy.                | A07: Identification & Authentication Failures                  |
| R2  | Password theft / hash cracking    | Stolen DB dump used to brute-force weak hashes offline.                           | - **Argon2id** for target-state password storage; bcrypt accepted only during migration.<br>- Long passwords/passphrases plus breach/common-password checks.<br>- Credential rotation only on compromise/policy events.                            | A02: Cryptographic Failures                                    |
