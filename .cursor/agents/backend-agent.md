...
**CRITICAL**: All backend code must be saved in the correct directories.
...
- **Backend modules**: `apps/backend/src/modules/<module>/`
  ...
  - Types: `apps/backend/src/modules/<module>/<module>.types.ts`
  - Schemas: `apps/backend/src/modules/<module>/<module>.schemas.ts`
  - Tests: `apps/backend/src/modules/<module>/__tests__/`
- **Database migrations**: `apps/backend/src/db/migrations/`
  - Format: `YYYYMMDDHHMM_description.ts` (e.g., `202501201430_create_user_profiles.ts`)
- **Database seeds**: `apps/backend/src/db/seeds/`
- **Shared utilities**: `apps/backend/src/utils/`
- **Configuration**: `apps/backend/src/config/`

**Rules**:
- Never save backend code in root directory
- Never save backend code in `.cursor/` directory
- Never save backend code outside `/apps/backend`
- Always use the correct subdirectory structure as defined above
- Follow the folder-by-module pattern for all modules
...
**Timestamp**: [ISO 8601 timestamp] (use current timestamp: `date -u +"%Y-%m-%dT%H:%M:%SZ"`)
...
- **Path**: `.cursor/agents/current_state/backend-agent-current_state.md`
- **Template**: `.cursor/agents/examples/current_state-template.md`

### State File Lifecycle

1. **Create**: When starting backend development work
...
2. **Update**: Continuously as development progresses
   - Update at least once per workflow phase (Analysis → Database → Implementation → Testing → Documentation)
   - Document completed steps, current phase, and progress
   - Track API endpoints, database changes, tests written, and issues encountered
...
- Current API endpoint or feature being developed
- Current workflow phase (1-5): Analysis → Database → Implementation → Testing → Documentation
- Files created/modified (controllers, services, repositories, migrations, tests)
...
```markdown
**Status**: in_progress
**Current Phase**: Phase 3 - Backend Module Implementation
**Completed Phases**: 1. Analysis ✅, 2. Database ✅ (migration created)
**Current Task**: Implementing PUT /api/v1/users/:id/profile endpoint
**Files Created**: 
  - apps/backend/src/modules/users/users.profile.controller.ts
  - apps/backend/src/modules/users/users.profile.service.ts
**Files Modified**:
  - apps/backend/src/modules/users/users.routes.ts (added route)
**Last Updated**: 2025-12-09T10:30:00Z
**Request ID**: BE-2025-12-09-001
```
...
**Keep inline** for:
- Complete module implementation patterns (Controller → Service → Repository)
- Database migration patterns with FitVibe-specific conventions
- Idempotency implementation examples
- API endpoint patterns with authentication/authorization
- Testing patterns specific to backend (unit, integration, repository tests)
- Code examples showing real FitVibe backend patterns

**Hybrid Approach**:
- ✅ **Reference shared examples** for standard handoff formats (see Handoff Protocol section)
- ✅ **Keep inline** for backend-agent-specific implementation patterns and code examples
...
- Request IDs: `"BE-YYYY-MM-DD-NNN"` (use current date: `date -u +"%Y-%m-%d"`)
